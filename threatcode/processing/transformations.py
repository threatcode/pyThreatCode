...
from threatcode.conditions import ConditionOR, ThreatcodeCondition
...
import re
import threatcode
from threatcode.rule import ThreatcodeLogSource, ThreatcodeRule, ThreatcodeDetection, ThreatcodeDetectionItem
from threatcode.exceptions import (
    ThreatcodeRegularExpressionError,
    ThreatcodeTransformationError,
    ThreatcodeValueError,
    ThreatcodeConfigurationError,
)
...
class Transformation(ABC):
    ...
    processing_item: Optional["threatcode.processing.pipeline.ProcessingItem"] = field(
    ...
    def from_dict(cls, d: dict) -> "Transformation":
        try:
            ...
            raise ThreatcodeConfigurationError("Error in instantiation of transformation: " + str(e))

    @abstractmethod
    def apply(
        self, pipeline: "threatcode.processing.pipeline.ProcessingPipeline", rule: ThreatcodeRule
    ) -> None:
        """Apply transformation on Threatcode rule."""
        self.pipeline: "threatcode.processing.pipeline.ProcessingPipeline" = (
            pipeline  # make pipeline accessible from all further options in class property
        )
        self.processing_item_applied(rule)

    def set_processing_item(self, processing_item: "threatcode.processing.pipeline.ProcessingItem"):
        self.processing_item = processing_item

    def processing_item_applied(
        self, d: Union[ThreatcodeRule, ThreatcodeDetection, ThreatcodeDetectionItem, ThreatcodeCondition]
    ):
...
class DetectionItemTransformation(Transformation):
    """
    Iterates over all detection items of a Threatcode rule and calls the apply_detection_item method
    ...
    object, which can be a ThreatcodeDetection or a ThreatcodeDetectionItem.
    ...
    ) -> Optional[Union[ThreatcodeDetection, ThreatcodeDetectionItem]]:
        """Apply transformation on detection item."""

    def apply_detection(self, detection: ThreatcodeDetection):
        for i, detection_item in enumerate(detection.detection_items):
            if isinstance(detection_item, ThreatcodeDetection):  # recurse into nested detection items
                ...
                if (
                    ...
                    if isinstance(r, ThreatcodeDetectionItem):
...
class FieldMappingTransformationBase(DetectionItemTransformation):
    """
    Transformation that is applied to detection items and additionally the field list of a Threatcode
    rule.
    """
    ...
    def apply_field_name(self, field: str) -> List[str]:
        """
        Apply field name transformation to a field list item of a Threatcode rule. It must always return
        a list of strings that are expanded into a new field list.
        """

    def _apply_field_name(
        self, pipeline: "threatcode.processing.pipeline.ProcessingPipeline", field: str
    ) -> List[str]:
        """
    ...
    def apply(
        ...
        """Apply field name transformations to Threatcode rule field names listed in 'fields' attribute."""
    ...
    def apply_detection_item(
        ...
        for value in detection_item.value:
            if self.processing_item is not None and self.processing_item.match_field_in_value(
                ...
                new_values.extend(
                    (
                        ThreatcodeFieldReference(mapped_field)
...
class ValueTransformation(DetectionItemTransformation):
    """
    Iterates over all values in all detection items of a Threatcode rule and call apply_value method
    ...
    def apply_detection_item(self, detection_item: ThreatcodeDetectionItem):
        ...
        for value in detection_item.value:
            if self.value_types is None or isinstance(
                ...
                elif isinstance(res, Iterable) and not isinstance(res, ThreatcodeType):
                    results.extend(res)
                    modified = True
                else:
                    results.append(res)
    ...
    @abstractmethod
    def apply_value(
        self, field: str, val: ThreatcodeType
    ) -> Optional[Union[ThreatcodeType, Iterable[ThreatcodeType]]]:
        """
        Perform a value transformation. This method can return:

        * None to drop the value
        * a single ThreatcodeType object which replaces the original value.
        * an iterable of ThreatcodeType objects. These objects are used as replacement for the
          original value.
...
class ConditionTransformation(Transformation):
    ...
    @abstractmethod
    def apply_condition(self, cond: ThreatcodeCondition) -> None:
        """
...
class DropDetectionItemTransformation(DetectionItemTransformation):
    ...
    class DeleteThreatcodeDetectionItem(ThreatcodeDetectionItem):
    ...
    def apply_detection_item(
        self, detection_item: ThreatcodeDetectionItem
    ) -> Optional[ThreatcodeDetectionItem]:
        """This function only marks detection items for deletion."""
        return self.DeleteThreatcodeDetectionItem.create()

    def apply_detection(self, detection: ThreatcodeDetection):
        super().apply_detection(detection)
        detection.detection_items = list(
            filter(
                lambda d: not isinstance(d, self.DeleteThreatcodeDetectionItem),
...
class BasePlaceholderTransformation(PlaceholderIncludeExcludeMixin, ValueTransformation):
    ...
    def apply_value(
        self, field: str, val: ThreatcodeString
    ) -> Union[ThreatcodeString, Iterable[ThreatcodeString]]:
    ...
    def placeholder_replacements(
        ...
        """
        Placeholder replacement callback used by ThreatcodeString.replace_placeholders(). This must return one
...
class ValueListPlaceholderTransformation(BasePlaceholderTransformation):
    ...
    def placeholder_replacements(self, p: Placeholder) -> List[str]:
        try:
            ...
            raise ThreatcodeValueError(f"Placeholder replacement variable '{ p.name }' doesn't exists.")
        ...
        return [ThreatcodeString(str(v)) for v in values]
    ...
    * mapping: Mapping between placeholders and identifiers that should be used in the expression.
    ...
    def apply_value(
        ...
        if val.contains_placeholder():
            if len(val.s) == 1:  # Threatcode string must only contain placeholder, nothing else.
                p = val.s[0]
                if self.is_handled_placeholder(p):
                    return ThreatcodeQueryExpression(self.expression, self.mapping.get(p.name) or p.name)
            else:  # ThreatcodeString contains placeholder as well as other parts
                raise ThreatcodeValueError(
...
class AddConditionTransformation(ConditionTransformation):
    ...
    * $category, $product and $service: with the corresponding values of the Threatcode rule log source.
    """
    ...
    def apply(
        ...
        rule.detection.detections[self.name] = ThreatcodeDetection.from_definition(conditions)
...
class ChangeLogsourceTransformation(Transformation):
    ...
    def apply(
        ...
        super().apply(pipeline, rule)
        logsource = ThreatcodeLogSource(self.category, self.product, self.service)
...
class ReplaceStringTransformation(ValueTransformation):
    ...
    capture groups. It operates on the plain string representation of the ThreatcodeString value.
    ...
    def apply_value(self, field: str, val: ThreatcodeString) -> ThreatcodeString:
        if isinstance(val, ThreatcodeString):
            return ThreatcodeString(self.re.sub(self.replacement, str(val)))
...
class SetStateTransformation(Transformation):
    ...
    def apply(self, pipeline: "threatcode.processing.pipeline.Proces", rule: ThreatcodeRule) -> None:
        super().apply(pipeline, rule)
...
class RuleFailureTransformation(Transformation):
    """
    Raise a ThreatcodeTransformationError with the provided message. This enables transformation
    ...
    def apply(
        ...
        raise ThreatcodeTransformationError(self.message)
...
class DetectionItemFailureTransformation(DetectionItemTransformation):
    ...
    def apply_detection_item(self, detection_item: ThreatcodeDetectionItem) -> None:
        raise ThreatcodeTransformationError(self.message)

...
from threatcode.processing.finalization import Finalizer, finalizers
from threatcode.processing.postprocessing import QueryPostprocessingTransformation
from threatcode.processing.tracking import FieldMappingTracking
from threatcode.rule import ThreatcodeDetectionItem, ThreatcodeRule
from threatcode.processing.transformations import transformations, Transformation
from threatcode.processing.postprocessing import query_postprocessing_transformations
from threatcode.processing.conditions import (
    rule_conditions,
...
from threatcode.exceptions import ThreatcodeConfigurationError, ThreatcodeTypeError
import yaml

from threatcode.types import ThreatcodeFieldReference, ThreatcodeType
...
class ProcessingItemBase:
    ...
    def _parse_conditions(
        ...
        for i, cond_def in enumerate(cond_defs):
            ...
            except (ThreatcodeConfigurationError, TypeError) as e:
                raise ThreatcodeConfigurationError(f"Error in condition { i + 1 }: { str(e) }") from e
        return conds
    ...
    def _instantiate_transformation(cls, d: dict, transformations: Dict[str, Type[Transformation]]):
        try:
            ...
            raise ThreatcodeConfigurationError("Missing transformation type")
    ...
    def match_rule_conditions(self, pipeline: "ProcessingPipeline", rule: ThreatcodeRule):
...
class ProcessingItem(ProcessingItemBase):
    ...
    the condition evaluates to true against the given Threatcode rule or if the condition is not present.
    ...
    def from_dict(cls, d: dict):
        ...
        try:
            ...
            raise ThreatcodeConfigurationError("Error in transformation: " + str(e)) from e
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.rule_conditions, list):
            raise ThreatcodeTypeError("Rule processing conditions must be provided as list")
        ...
        if not isinstance(self.detection_item_conditions, list):
            raise ThreatcodeTypeError("Detection item processing conditions must be provided as list")
        ...
        if not isinstance(self.field_name_conditions, list):
            raise ThreatcodeTypeError("Field name processing conditions must be provided as list")
    ...
    def apply(self, pipeline: "ProcessingPipeline", rule: ThreatcodeRule) -> bool:
        """
        Matches condition against rule and performs transformation if condition is true or not present.
        Returns Threatcode rule and bool if transformation was applied.
        """
    ...
    def match_detection_item(
        self, pipeline: "ProcessingPipeline", detection_item: ThreatcodeDetectionItem
    ) -> bool:
        """
    ...
    def match_field_in_value(self, pipeline: "ProcessingPipeline", value: ThreatcodeType) -> bool:
        """
        Evaluate field name conditions in field reference values and return result.
        """
        if isinstance(value, ThreatcodeFieldReference):
            field_name_cond_result = self.field_name_condition_linking(
...
class QueryPostprocessingItem(ProcessingItemBase):
    ...
    given Threatcode rule or if the condition is not present.
    ...
    def apply(
        self, pipeline: "ProcessingPipeline", rule: ThreatcodeRule, query: str
    ) -> Tuple[str, bool]:
        """
...
class ProcessingPipeline:
    """
    A processing pipeline is configured with the transformation steps that are applied on Threatcode rules and
    are configured by:

    * a backend to apply a set of base preprocessing of Threatcode rules (e.g. renaming of fields).
    * the user in one or multiple configurations to conduct further rule transformation to adapt the rule
    ...
    )  # Set of identifiers of backends (from the backends mapping) that are allowed to use this processing pipeline. This can be used by frontends like Threatcode CLI to warn the user about inappropriate usage.
    ...
    def from_dict(cls, d: dict) -> "ProcessingPipeline":
        ...
        for i, item in enumerate(items):
            ...
            except ThreatcodeConfigurationError as e:
                raise ThreatcodeConfigurationError(
        ...
        for fd in fds:
            ...
            try:
                ...
                raise ThreatcodeConfigurationError(f"Finalizer '{finalizer_type}' is unknown")
    ...
    def apply(self, rule: ThreatcodeRule) -> ThreatcodeRule:
        """Apply processing pipeline on Threatcode rule."""
        self.applied = list()
    ...
    def postprocess_query(self, rule: ThreatcodeRule, query: Any) -> Any:

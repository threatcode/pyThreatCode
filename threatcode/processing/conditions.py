...
import threatcode
from threatcode.types import ThreatcodeFieldReference, ThreatcodeString, ThreatcodeType, threatcode_type
from typing import Dict, List, Pattern, Literal, Optional, Union
import re
from threatcode.rule import ThreatcodeDetection, ThreatcodeRule, ThreatcodeDetectionItem, ThreatcodeLogSource
from threatcode.exceptions import ThreatcodeConfigurationError, ThreatcodeRegularExpressionError
...
class RuleProcessingCondition(ProcessingCondition, ABC):
    """
    Base for Threatcode rule processing condition classes used in processing pipelines.
    """
    ...
    def match(
        ...
        """Match condition on Threatcode rule."""


class FieldNameProcessingCondition(ProcessingCondition, ABC):
    """
    Base class for conditions on field names in detection items, Threatcode rule field lists and other
    ...
    def match_detection_item(
        self,
        pipeline: "threatcode.processing.pipeline.ProcessingPipeline",
    ...
    def match_value(
        self, pipeline: "threatcode.processing.pipeline.ProcessingPipeline", value: ThreatcodeType
    ) -> bool:
        """
...
class DetectionItemProcessingCondition(ProcessingCondition, ABC):
    """
    Base for Threatcode detection item processing condition classes used in processing pipelines.
    """
...
class LogsourceCondition(RuleProcessingCondition):
    ...
    def __post_init__(self):
        self.logsource = ThreatcodeLogSource(self.category, self.product, self.service)
...
class RuleContainsDetectionItemCondition(RuleProcessingCondition):
    ...
    def __post_init__(self):
        self.threatcode_value = threatcode_type(self.value)
    ...
    def find_detection_item(self, detection: Union[ThreatcodeDetectionItem, ThreatcodeDetection]) -> bool:
        if isinstance(detection, ThreatcodeDetection):
            for detection_item in detection.detection_items:
                if self.find_detection_item(detection_item):
                    return True
        elif isinstance(detection, ThreatcodeDetectionItem):
            if (
                detection.field is not None
                and detection.field == self.field
                and self.threatcode_value
                in [v for v in detection.value if type(self.threatcode_value) == type(v)]
            ):
                return True
        else:
            raise TypeError("Parameter of type ThreatcodeDetection or ThreatcodeDetectionItem expected.")
...
class MatchStringCondition(ValueProcessingCondition):
    ...
    def match_value(
        ...
        if isinstance(value, ThreatcodeString):

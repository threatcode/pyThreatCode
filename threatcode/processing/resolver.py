...
from threatcode.exceptions import (
    ThreatcodePipelineNotAllowedForBackendError,
    ThreatcodePipelineNotFoundError,
)
...
class ProcessingPipelineResolver:
    ...
    def resolve_pipeline(self, spec: str, target: Optional[str] = None) -> ProcessingPipeline:
        ...
        too, a *ThreatcodePipelineNotFoundError* is raised.

        If *target* is specified, an additional check of the compatibility of the specified backend
        to the resolved pipeline is conducted. A *ThreatcodePipelineNotAllowedForBackendError* is raised
        if this check fails.
        """
        try:

...
from threatcode.processing.transformations import Transformation
...
class QueryPostprocessingTransformation(Transformation):
    ...
    processing_item: Optional["threatcode.processing.pipeline.QueryPostprocessingItem"] = field(
        init=False, compare=False, default=None
    )

    @abstractmethod
    def apply(
        self, pipeline: "threatcode.processing.pipeline.ProcessingPipeline", rule: ThreatcodeRule, query: Any
    ) -> Any:
        ...
        :param rule: Threatcode rule that is associated with the generated query.
...
class EmbedQueryTransformation(QueryPostprocessingTransformation):
    ...
    def apply(
        self, pipeline: "threatcode.processing.pipeline.ProcessingPipeline", rule: ThreatcodeRule, query: str
    ) -> str:
...
class QuerySimpleTemplateTransformation(QueryPostprocessingTransformation):
    ...
    * rule: the Threatcode rule including all its attributes like rule.title.

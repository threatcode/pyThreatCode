...
from threatcode.types import ThreatcodeType
from threatcode.exceptions import ThreatcodeConditionError, ThreatcodeRuleLocation
import threatcode
...
class ParentChainMixin:
    ...
    def postprocess(
        self,
        detections: "threatcode.rule.ThreatcodeDetections",
...
class ConditionIdentifier(ConditionItem):
    ...
    def postprocess(
        ...
        """Converts an identifier into a condition with ThreatcodeDetectionItems at its leaf nodes."""
        self.parent = parent
        try:
            detection = detections[self.identifier]
        except KeyError:
            raise ThreatcodeConditionError(
...
class ConditionSelector(ConditionItem):
    ...
    def resolve_referenced_detections(self, detections: "threatcode.rule.ThreatcodeDetections") -> List[str]:
        """
...
@dataclass
class ThreatcodeCondition(ProcessingItemTrackingMixin):
    condition: str
    detections: "threatcode.rule.ThreatcodeDetections"
    source: Optional[ThreatcodeRuleLocation] = field(default=None, compare=False)
    ...
    def parse(self, postprocess: bool = True):
        ...
        :raises ThreatcodeConditionError: on parse error.
        :return: Parse or condition tree.
        """
        if "|" in self.condition:
            raise ThreatcodeConditionError(
                "The pipe syntax in Threatcode conditions will be deprecated and replaced by Threatcode correlations. pyThreatcode doesn't supports this syntax."
            )
        try:
            ...
            raise ThreatcodeConditionError(str(e), source=self.source)

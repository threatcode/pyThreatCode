...
from threatcode.exceptions import ThreatcodeCollectionError, ThreatcodeError, ThreatcodeRuleLocation
import yaml


@dataclass
class ThreatcodeCollection:
    """Collection of Threatcode rules"""

    rules: List[ThreatcodeRule]
    errors: List[ThreatcodeError] = field(default_factory=list)
    ids_to_rules: Dict[str, ThreatcodeRule] = field(init=False, repr=False, hash=False, compare=False)
    ...
    def from_dicts(
        ...
        in the errors property individually for each Threatcode rule and the whole ThreatcodeCollection.
        """
        errors = []
        parsed_rules = list()
        prev_rule = None
        ...
        for i, rule in zip(range(1, len(rules) + 1), rules):
            if isinstance(
                ...
                if (
                    ...
                    parsed_rules.append(ThreatcodeRule.from_dict(prev_rule, collect_errors, source))
                else:
                    exception = ThreatcodeCollectionError(
                        f"Unknown Threatcode collection action '{ action }' in rule { i }",
                        source=source,
                    )
                    if collect_errors:
    ...
    def load_ruleset(
        ...
        on_load: Optional[Callable[[Path, "ThreatcodeCollection"], Optional["ThreatcodeCollection"]]] = None,
        recursion_pattern: str = "**/*.yml",
    ) -> "ThreatcodeCollection":
        """
        Load a ruleset from a list of files or directories and construct a :class:`ThreatcodeCollection`
        object.

        :param inputs: List of strings and :class:`pathlib.Path` objects that reference files or
        directories that should be loaded.
        :param collect_errors: parse or verification errors are collected in :class:`ThreatcodeRule`
        objects instead of raising them immediately. Defaults to ``False``.
        :param on_beforeload: Optional function that is called for each path to a Threatcode rule before the parsing and
        construction of the :class:`ThreatcodeCollection` object is done. The path returned by this function is
        used as input. A rule path is skipped if ``None`` is returned.
        :param on_load: Optional function that is called after the :class:`ThreatcodeCollection` was
        constructed from the path. The path and the ThreatcodeCollection object are passed to this
        function and it is expected to return a :class:`ThreatcodeCollection` object that is merged in
        ...
        :return: :class:`ThreatcodeCollection` of all threatcode rules contained in given paths.
        """
        ...
        for path in paths:
            ...
            if path is not None:  # Skip if path is None
                threatcode_collection = ThreatcodeCollection.from_yaml(
                    path.open(encoding="utf-8"), collect_errors, ThreatcodeRuleLocation(path)
                )
                if (
                    on_load is not None
                ):  # replace ThreatcodeCollection generated from file content with the return value from on_load function if provided
                    threatcode_collection = on_load(path, threatcode_collection)
                if threatcode_collection is not None:  # Skip if nothing
                    threatcode_collections.append(threatcode_collection)

        # Finally merge all ThreatcodeCollection's and return the result
        return cls.merge(threatcode_collections)

    @classmethod
    def merge(cls, collections: Iterable["ThreatcodeCollection"]) -> "ThreatcodeCollection":
        """Merge multiple ThreatcodeCollection objects into one and return it."""
        return cls(

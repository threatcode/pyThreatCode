...
import yaml
import threatcode
from threatcode.types import ThreatcodeType, ThreatcodeNull, ThreatcodeString, ThreatcodeNumber, threatcode_type
from threatcode.modifiers import (
    ThreatcodeModifier,
    modifier_mapping,
    reverse_modifier_mapping,
    ThreatcodeValueModifier,
    ThreatcodeListModifier,
)
from threatcode.conditions import (
    ThreatcodeCondition,
    ConditionAND,
    ConditionOR,
...
from threatcode.processing.tracking import ProcessingItemTrackingMixin
import threatcode.exceptions as threatcode_exceptions
from threatcode.exceptions import (
    ThreatcodeRuleLocation,
    ThreatcodeTypeError,
    ThreatcodeError,
)
...
class ThreatcodeStatus(EnumLowercaseStringMixin, Enum):
    STABLE = auto()
    EXPERIMENTAL = auto()
    TEST = auto()
    DEPRECATED = auto()
...
class ThreatcodeLevel(EnumLowercaseStringMixin, Enum):
    INFORMATIONAL = auto()
    LOW = auto()
    MEDIUM = auto()
    HIGH = auto()
...
class ThreatcodeRelatedType(EnumLowercaseStringMixin, Enum):
    OBSOLETES = auto()
    RENAMED = auto()
    MERGED = auto()
    DERIVED = auto()
...
@dataclass
class ThreatcodeRelatedItem:
    id: UUID
    type: ThreatcodeRelatedType

    @classmethod
    def from_dict(cls, value: dict) -> "ThreatcodeRelatedItem":
        """Returns Related item from dict with fields."""
        try:
            id = UUID(value["id"])
        except ValueError:
            raise threatcode_exceptions.ThreatcodeRelatedError(f"Threatcode related identifier must be an UUID")

        try:
            type = ThreatcodeRelatedType[value["type"].upper()]
        except:
            raise threatcode_exceptions.ThreatcodeRelatedError(
                f"{value['type']} is not a Threatcode related valid type"
            )
...
@dataclass
class ThreatcodeRelated:
    related: List[Type[ThreatcodeRelatedItem]]

    @classmethod
    def from_dict(cls, val: list) -> "ThreatcodeRelated":
        """Returns Related object from dict with fields."""

        list_ret = []
        for v in val:
            if not "id" in v.keys():
                raise threatcode_exceptions.ThreatcodeRelatedError("Threatcode related must have an id field")
            elif not "type" in v.keys():
                raise threatcode_exceptions.ThreatcodeRelatedError("Threatcode related must have a type field")
            else:
                list_ret.append(ThreatcodeRelatedItem.from_dict(v))  # should rise the ThreatcodeRelatedError

        return cls(list_ret)


@dataclass(unsafe_hash=True)
class ThreatcodeRuleTag:
    namespace: str
    name: str
    source: Optional[ThreatcodeRuleLocation] = field(default=None, compare=False)

    @classmethod
    def from_str(cls, tag: str, source: Optional[ThreatcodeRuleLocation] = None) -> "ThreatcodeRuleTag":
        """Build ThreatcodeRuleTag class from plain text tag string."""
        try:
            ns, n = tag.split(".", maxsplit=1)
        except ValueError:
            raise threatcode_exceptions.ThreatcodeValueError(
                "Threatcode tag must start with namespace separated with dot from remaining tag."
            )
        return cls(ns, n)
...
class ThreatcodeLogSource:
    ...
    def __post_init__(self):
        ...
        if self.category is None and self.product is None and self.service is None:
            raise threatcode_exceptions.ThreatcodeLogsourceError(
                "Threatcode log source can't be empty", source=self.source
            )

    @classmethod
    def from_dict(
        cls, logsource: dict, source: Optional[ThreatcodeRuleLocation] = None
    ) -> "ThreatcodeLogSource":
        """Returns ThreatcodeLogSource object from dict with fields."""
        return cls(
    ...
    def __contains__(self, other: "ThreatcodeLogSource") -> bool:
        """
...
@dataclass
class ThreatcodeDetectionItem(ProcessingItemTrackingMixin, ParentChainMixin):
    """
    Single Threatcode detection definition
    ...
    * the mandatory value or a list of values (internally it's always a list of values)
    ...
    modifiers: List[Type[ThreatcodeModifier]]
    value: List[ThreatcodeType]
    value_linking: Union[Type[ConditionAND], Type[ConditionOR]] = ConditionOR
    source: Optional[ThreatcodeRuleLocation] = field(default=None, compare=False)
    original_value: Optional[ThreatcodeType] = field(
    ...
    def __post_init__(self, auto_modifiers):
        ...
        self.value = [  # value cleanup: convert plain values into ThreatcodeType's
            threatcode_type(v) if not isinstance(v, ThreatcodeType) else v for v in self.value
        ]
    ...
    def apply_modifiers(self):
        ...
        for modifier in self.modifiers:
            ...
            if isinstance(
                modifier_instance, ThreatcodeValueModifier
                ...
                modifier_instance, ThreatcodeListModifier
            ):  # List modifiers are applied to the whole value list at once
                self.value = modifier_instance.apply(self.value)
            else:  # pragma: no cover
                raise TypeError(
                    "Instance of ThreatcodeValueModifier or ThreatcodeListModifier was expected",
                    source=self.source,
    ...
    def from_mapping(
    ...
    ) -> "ThreatcodeDetectionItem":
        """
        Constructs ThreatcodeDetectionItem object from a mapping between field name containing
        ...
        the value list always contained in a ThreatcodeDetectionItem instance.
        """
        ...
        try:
            ...
            raise threatcode_exceptions.ThreatcodeModifierError(f"Unknown modifier {str(e)}", source=source)
        ...
        # Map Python types to Threatcode typing classes
        val = [threatcode_type(v) for v in val]
    ...
    def postprocess(
        self,
        detections: "ThreatcodeDetections",
        parent: Optional["threatcode.condition.ConditionItem"] = None,
    ) -> Union[
        ConditionAND,
        ConditionOR,
        ConditionFieldEqualsValueExpression,
        ...
        if len(self.value) == 0:  # no value: map to none type
            if self.field is None:
                raise threatcode_exceptions.ThreatcodeConditionError(
                    "Null value must be bound to a field", source=self.source
                )
            else:
                return ConditionFieldEqualsValueExpression(self.field, ThreatcodeNull()).postprocess(
...
@dataclass
class ThreatcodeDetection(ParentChainMixin):
    """
    A detection is a set of atomic event defitionions represented by ThreatcodeDetectionItem instances. ThreatcodeDetectionItems
    of a ThreatcodeDetection are OR-linked.
    ...
    detection_items: List[Union[ThreatcodeDetectionItem, "ThreatcodeDetection"]]
    source: Optional[ThreatcodeRuleLocation] = field(default=None, compare=False)
    ...
    def __post_init__(self):
        ...
        if len(self.detection_items) == 0:
            raise threatcode_exceptions.ThreatcodeDetectionError("Detection is empty", source=self.source)

        if self.item_linking is None:
            type_set = {type(item) for item in self.detection_items}
            if ThreatcodeDetectionItem in type_set:
    ...
    def from_definition(
        ...
        """Instantiate an appropriate ThreatcodeDetection object from a parsed Threatcode detection definition."""
        if isinstance(definition, Mapping):  # key-value-definition (case 1)
            return cls(
                detection_items=[
                    ThreatcodeDetectionItem.from_mapping(key, val, source)
            ...
            return cls(
                detection_items=[ThreatcodeDetectionItem.from_value(definition, source)],
                source=source,
            )
            ...
            if {type(item) for item in definition}.issubset(
                ...
                return cls(
                    detection_items=[
                        ThreatcodeDetection.from_definition(
                            item, source
                        )  # nested ThreatcodeDetection in other cases
    ...
    def add_applied_processing_item(
        self, processing_item: Optional["threatcode.processing.pipeline.ProcessingItem"]
    ):
...
@dataclass
class ThreatcodeDetections:
    """Threatcode detection section including named detections and condition."""

    detections: Dict[str, ThreatcodeDetection]
    condition: List[str]
    source: Optional[ThreatcodeRuleLocation] = field(default=None, compare=False)
    ...
    def __post_init__(self):
        ...
        if self.detections == dict():
            raise threatcode_exceptions.ThreatcodeDetectionError(
                "No detections defined in Threatcode rule", source=self.source
            )
        self.parsed_condition = [ThreatcodeCondition(cond, self, self.source) for cond in self.condition]

    @classmethod
    def from_dict(
        cls, detections: dict, source: Optional[ThreatcodeRuleLocation] = None
    ) -> "ThreatcodeDetections":
        try:
            ...
            raise threatcode_exceptions.ThreatcodeConditionError(
                "Threatcode rule must contain at least one condition", source=source
            )

        return cls(
            detections={
                name: ThreatcodeDetection.from_definition(definition, source)
    ...
    def __getitem__(self, key: str) -> ThreatcodeDetection:
        """Get detection by name"""
        return self.detections[key]


class ThreatcodeYAMLLoader(yaml.SafeLoader):
    """Custom YAML loader implementing additional functionality for Threatcode."""
...
@dataclass
class ThreatcodeRule(ProcessingItemTrackingMixin):
    """
    A single Threatcode rule.
    """

    title: str
    logsource: ThreatcodeLogSource
    detection: ThreatcodeDetections
    id: Optional[UUID] = None
    related: Optional[ThreatcodeRelated] = None
    status: Optional[ThreatcodeStatus] = None
    description: Optional[str] = None
    references: List[str] = field(default_factory=list)
    tags: List[ThreatcodeRuleTag] = field(default_factory=list)
    author: Optional[str] = None
    date: Optional[date] = None
    modified: Optional[date] = None
    fields: List[str] = field(default_factory=list)
    falsepositives: List[str] = field(default_factory=list)
    level: Optional[ThreatcodeLevel] = None

    errors: List[threatcode_exceptions.ThreatcodeError] = field(default_factory=list)
    source: Optional[ThreatcodeRuleLocation] = field(default=None, compare=False)
    ...
    def from_dict(
        ...
        """
        Convert Threatcode rule parsed in dict structure into ThreatcodeRule object.

        if collect_errors is set to False exceptions are collected in the errors property of the resulting
        ThreatcodeRule object. Else the first recognized error is raised as exception.
        """
        errors = []
        ...
        if rule_id is not None:
            try:
                ...
                errors.append(
                    threatcode_exceptions.ThreatcodeIdentifierError(
                        "Threatcode rule identifier must be an UUID", source=source
                    )
                )
        ...
        if rule_related is not None:
            if not isinstance(rule_related, list):
                errors.append(
                    threatcode_exceptions.ThreatcodeRelatedError(
                        "Threatcode rule related must be a list", source=source
                    )
                )
            else:
                try:
                    rule_related = ThreatcodeRelated.from_dict(rule_related)
                except threatcode_exceptions.ThreatcodeRelatedError as e:
                    errors.append(e)
        ...
        if level is not None:
            try:
                level = ThreatcodeLevel[level.upper()]
            except KeyError:
                errors.append(
                    threatcode_exceptions.ThreatcodeLevelError(
                        f"'{ level }' is no valid Threatcode rule level", source=source
                    )
                )
        ...
        if status is not None:
            if not isinstance(status, str):
                errors.append(
                    threatcode_exceptions.ThreatcodeStatusError(
                        "Threatcode rule status cannot be a list", source=source
                    )
                )
            else:
                try:
                    status = ThreatcodeStatus[status.upper()]
                except KeyError:
                    errors.append(
                        threatcode_exceptions.ThreatcodeStatusError(
                            f"'{ status }' is no valid Threatcode rule status", source=source
                        )
                    )
            ...
            if not isinstance(rule_date, date) and not isinstance(rule_date, datetime):
                try:
                    ...
                    try:
                        ...
                        errors.append(
                            threatcode_exceptions.ThreatcodeDateError(
            ...
            if not isinstance(rule_modified, date) and not isinstance(rule_modified, datetime):
                try:
                    ...
                    try:
                        ...
                        errors.append(
                            threatcode_exceptions.ThreatcodeModifiedError(
        ...
        if rule_fields is not None and not isinstance(rule_fields, list):
            errors.append(
                threatcode_exceptions.ThreatcodeFieldsError(
                    "Threatcode rule fields must be a list",
                    source=source,
                )
            )
        ...
        if rule_falsepositives is not None and not isinstance(rule_falsepositives, list):
            errors.append(
                threatcode_exceptions.ThreatcodeFalsePositivesError(
                    "Threatcode rule falsepositives must be a list",
                    source=source,
                )
            )
        ...
        if rule_author is not None and not isinstance(rule_author, str):
            errors.append(
                threatcode_exceptions.ThreatcodeAuthorError(
                    "Threatcode rule author must be a string",
                    source=source,
                )
            )
        ...
        if rule_description is not None and not isinstance(rule_description, str):
            errors.append(
                threatcode_exceptions.ThreatcodeDescriptionError(
                    "Threatcode rule description must be a string",
                    source=source,
                )
            )
        ...
        if rule_references is not None and not isinstance(rule_references, list):
            errors.append(
                threatcode_exceptions.ThreatcodeReferencesError(
                    "Threatcode rule references must be a list",
                    source=source,
                )
            )
        ...
        if rule_title is None:
            errors.append(
                threatcode_exceptions.ThreatcodeTitleError(
                    "Threatcode rule must have a title",
                    source=source,
                )
            )
            ...
            errors.append(
                threatcode_exceptions.ThreatcodeTitleError(
                    "Threatcode rule title must be a string",
                    source=source,
                )
            )
        ...
        logsource = None
        try:
            logsource = ThreatcodeLogSource.from_dict(rule["logsource"], source)
        except KeyError:
            errors.append(
                threatcode_exceptions.ThreatcodeLogsourceError(
                    "Threatcode rule must have a log source", source=source
                )
            )
        ...
        detections = None
        try:
            detections = ThreatcodeDetections.from_dict(rule["detection"], source)
        except KeyError:
            errors.append(
                threatcode_exceptions.ThreatcodeDetectionError(
                    "Threatcode rule must have a detection definitions", source=source
                )
            )
        except ThreatcodeError as e:
            errors.append(e)
        ...
        return cls(
            ...
            tags=[ThreatcodeRuleTag.from_str(tag) for tag in rule.get("tags", list())],
            author=rule_author,
            date=rule_date,
    ...
    @classmethod
    def from_yaml(cls, rule: str, collect_errors: bool = False) -> "ThreatcodeRule":
        """Convert YAML input string with single document into ThreatcodeRule object."""
        parsed_rule = yaml.load(rule, ThreatcodeYAMLLoader)

...
from threatcode.exceptions import (
    ThreatcodePlaceholderError,
    ThreatcodeRuleLocation,
    ThreatcodeValueError,
    ThreatcodeRegularExpressionError,
    ThreatcodeTypeError,
)
...
class Placeholder:
    """
    Placeholder class used as stub in a ThreatcodeString to be later replaced by a value contained in a string or
...
class ThreatcodeType(ABC):
    """Base class for Threatcode value types"""

    def to_plain(self):
        """
        Return plain Python value (str, int etc.) from ThreatcodeType instance for usage in conversion of
        Threatcode rules back to dicts. Uses the first annotated member as return value.
        """
        return self.__getattribute__(list(self.__annotations__.keys())[0])


class NoPlainConversionMixin:
    """Mixin for declaring a ThreatcodeType as non-convertible into a plain representation."""

    def to_plain(self):
        raise ThreatcodeValueError(
            f"Threatcode type '{ self.__class__.__name__ }' can't be converted into a plain representation."
        )


class ThreatcodeNull(ThreatcodeType):
    ...
    def __eq__(self, other: "ThreatcodeNull") -> bool:
        return isinstance(other, self.__class__)


@dataclass
class ThreatcodeExists(ThreatcodeType):
...
class ThreatcodeString(ThreatcodeType):
    """
    Strings in Threatcode detection values containing wildcards.
    """
    ...
    def __init__(self, s: Optional[str] = None):
        """
        Initializes ThreatcodeString instance from raw string by parsing it:
    ...
    def __getitem__(self, idx: Union[int, slice]) -> "ThreatcodeString":
        """
        Index ThreatcodeString parts with transparent handling of special characters.

        :param key: Integer index or slice.
        :type key: Union[int, slice]
        :return: ThreatcodeString containing only the specified part.
        :rtype: ThreatcodeString
        """
        ...
        if isinstance(idx, int):
            ...
            if idx.step is not None:
                raise IndexError("ThreatcodeString slice index with step is not allowed")
            start = idx.start or 0
            end = idx.stop or inf
        else:
            raise TypeError("ThreatcodeString indices must be integers or slices")
        ...
        if start > end or start >= length:
            return ThreatcodeString("")
        if start < 0 or end < 0 or (end != inf and end > length):
            raise IndexError("ThreatcodeString index out of range")

        i = 0  # Pointer to ThreatcodeString element
        ...
        while start > 0 and i < len(self.s):
            e = self.s[i]
            if isinstance(e, str):  # Current ThreatcodeString part is string
                e_len = len(e)
                # if e_len <= start:
                if e_len > start:
                    # else:
                    if end < e_len:  # end lies within this string part
                        return ThreatcodeString(e[start:end])
            ...
            else:  # Current ThreatcodeString part is a special character or placeholder
                start -= 1
                end -= 1
    ...
    def insert_placeholders(self) -> "ThreatcodeString":
        """
    ...
    def replace_with_placeholder(self, regex: Pattern, placeholder_name: str) -> "ThreatcodeString":
        """
    ...
    def _merge_strs(self) -> "ThreatcodeString":
    ...
    def __add__(self, other: Union["ThreatcodeString", str, SpecialChars, Placeholder]) -> "ThreatcodeString":
        s = self.__class__()
    ...
    def __radd__(self, other: Union[str, SpecialChars, Placeholder]) -> "ThreatcodeString":
    ...
    def __eq__(self, other: Union["ThreatcodeString", str]) -> bool:
        if isinstance(other, str):
            ...
            raise NotImplementedError(
                "ThreatcodeString can only be compared with a string or another ThreatcodeString"
            )
    ...
    def __str__(self) -> str:
        ...
        for s in self.s:
            if isinstance(s, str):
                ...
                raise TypeError(
                    "ThreatcodeString can only consist of plain strings and instances of SpecialChars or Placeholder objects."
                )
        return rs
    ...
    def to_plain(self):
        """Return plain string representation of ThreatcodeString, equivalent to converting it with str()."""
        return str(self)
    ...
    def replace_placeholders(
        ...
        is concatenated to the ThreatcodeString prefix before the placeholder and the method is called recursively with the suffix
        after the placeholder. All placeholder replacements are combined with all returned ThreatcodeString suffixes. Therefore,
        ...
        for i in range(len(s)):
            if isinstance(s[i], Placeholder):  # Placeholder instance at index, do replacement
                prefix = ThreatcodeString()
                prefix.s = s[:i]
                placeholder = s[i]
                suffix = ThreatcodeString()
                suffix.s = s[i + 1 :]
                return [
                    ...
                    for result_suffix in suffix.replace_placeholders(
                        callback
                    )  # iterate over all result values of calling this method with the ThreatcodeString remainder
                ]
    ...
    def convert(
        ...
        """
        Convert ThreatcodeString into a query string or pattern. The following parameters allow to change the behavior:
        ...
        * filter_chars: characters that are filtered out.
        ...
        of these characters in a string will raise a ThreatcodeValueError.
        """
        s = ""
        ...
        for c in self:
            if isinstance(c, str):  # c is plain character
                ...
                raise ThreatcodePlaceholderError(
                    f"Attempt to convert unhandled placeholder '{c.name}' into query."
                )
            else:
                raise ThreatcodeValueError(
                    f"Trying to convert ThreatcodeString containing part of type '{type(c).__name__}'"
                )
        return s


class ThreatcodeCasedString(ThreatcodeString):
    """Case-sensitive string matching."""

    @classmethod
    def from_threatcode_string(cls, s: ThreatcodeString) -> "ThreatcodeCasedString":
        cs = cls(s.original)
        cs.s = s.s
        return cs
...
@dataclass
class ThreatcodeNumber(ThreatcodeType):
    """Numeric value type"""
    ...
    def __post_init__(self):
        try:  # Only use float number if it can't be represented as int.
            ...
            raise ThreatcodeValueError("Invalid number") from e

    def __str__(self):
        return str(self.number)

    def __eq__(self, other: Union["ThreatcodeNumber", int]) -> bool:
...
@dataclass
class ThreatcodeBool(ThreatcodeType):
    """Boolean value type"""
    ...
    def __post_init__(self):
        if not isinstance(self.boolean, bool):
            raise ThreatcodeTypeError("ThreatcodeBool must be a boolean")
...
class ThreatcodeRegularExpressionFlag(Enum):
    IGNORECASE = auto()
    MULTILINE = auto()
    DOTALL = auto()
...
@dataclass
class ThreatcodeRegularExpression(ThreatcodeType):
    """Regular expression type"""

    regexp: str
    flags: Set[ThreatcodeRegularExpressionFlag] = field(default_factory=set)
    threatcode_to_python_flags: ClassVar[Dict[ThreatcodeRegularExpressionFlag, re.RegexFlag]] = {
        ThreatcodeRegularExpressionFlag.IGNORECASE: re.IGNORECASE,
        ThreatcodeRegularExpressionFlag.MULTILINE: re.MULTILINE,
        ThreatcodeRegularExpressionFlag.DOTALL: re.DOTALL,
    }
    threatcode_to_re_flag: ClassVar[Dict[ThreatcodeRegularExpressionFlag, str]] = {
        ThreatcodeRegularExpressionFlag.IGNORECASE: "i",
        ThreatcodeRegularExpressionFlag.MULTILINE: "m",
        ThreatcodeRegularExpressionFlag.DOTALL: "s",
    }

    def __post_init__(self):
        self.compile()

    def add_flag(self, flag: ThreatcodeRegularExpressionFlag):
        self.flags.add(flag)
    ...
    def compile(self):
        ...
        try:
            ...
            for flag in self.flags:
                flags |= self.threatcode_to_python_flags[flag]
            re.compile(self.regexp, flags)
        except re.error as e:
            raise ThreatcodeRegularExpressionError(
    ...
    def escape(
        ...
        if flag_prefix and self.flags:
            prefix = (
                "(?" + "".join(sorted((self.threatcode_to_re_flag[flag] for flag in self.flags))) + ")"
            )
        else:
            prefix = ""
...
@dataclass
class ThreatcodeCIDRExpression(NoPlainConversionMixin, ThreatcodeType):
    ...
    def __post_init__(self):
        ...
        try:
            ...
            raise ThreatcodeTypeError("Invalid CIDR expression: " + str(e), source=self.source)
    ...
    def expand(
        ...
        * wildcard: string that should be output as wildcard. Usually not required because this is
          passed to ThreatcodeString that generates a wildcard pecial character from '*' that is
...
@dataclass
class ThreatcodeCompareExpression(NoPlainConversionMixin, ThreatcodeType):
    ...
    number: ThreatcodeNumber
    op: CompareOperators
    source: Optional[ThreatcodeRuleLocation] = None

    def __post_init__(self):
        if not isinstance(self.number, ThreatcodeNumber):
            raise ThreatcodeTypeError("Compare operator expects number", source=self.source)


@dataclass
class ThreatcodeFieldReference(NoPlainConversionMixin, ThreatcodeType):
...
@dataclass
class ThreatcodeQueryExpression(NoPlainConversionMixin, ThreatcodeType):
    """
    ...
    def __post_init__(self):
        if not isinstance(self.expr, str):
            raise ThreatcodeTypeError("ThreatcodeQueryExpression expression must be a string")
        if not isinstance(self.id, str):
            raise ThreatcodeTypeError("ThreatcodeQueryExpression placeholder identifier must be a string")
...
@dataclass
class ThreatcodeExpansion(NoPlainConversionMixin, ThreatcodeType):
    """
    Special purpose type for correct logic linking of values expanded by modifiers. In the usual
    cases the writer of a Threatcode rule expects the values expanded by modifiers like base64offset or
    windash are OR-linked, even if the value list containing the original values is linked with AND
    by modifying it with 'all'. A ThreatcodeExpansion is emitted by such modifiers, contains the
    ...
    values: List[ThreatcodeType]


type_map = {
    bool: ThreatcodeBool,
    int: ThreatcodeNumber,
    float: ThreatcodeNumber,
    str: ThreatcodeString,
    type(None): ThreatcodeNull,
}


def threatcode_type(v: Optional[Union[int, float, str]]):
    """Return Threatcode type from Python value"""

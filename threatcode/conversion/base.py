...
from threatcode.exceptions import ThreatcodeError, ThreatcodeValueError
from threatcode.conversion.deferred import DeferredQueryExpression
...
from threatcode.conversion.state import ConversionState


class Backend(ABC):
    """
    Base class for Threatcode conversion backends. A backend is made up from the following elements:

    * Some metadata about the properties of the backend.
    * A processing pipeline stored in backend_processing_pipeline that is applied to each Threatcode
      rule that is converted by the backend. This is the location where you add generic
      transformations that should be applied to all Threatcode rules before conversion.
    ...
    * If collect_errors is set to True, exceptions will not be thrown, but collected in (threatcode_rule, exception)
    ...
    * finalize_output_<format> finalizes the conversion result of a whole rule set in the specified format.
    ...
    2. If required, implement a per-query finalization step in finalize_query. Each Threatcode rule condition
    ...
    * Use intermediate representations for queries and query sets for formats that require state information,
    ...
    errors: List[Tuple[ThreatcodeRule, ThreatcodeError]]
    ...
    def convert(self, rule_collection: ThreatcodeCollection, output_format: Optional[str] = None) -> Any:
        """
        Convert a Threatcode ruleset into the target data structure. Usually the result are one or
    ...
    def convert_rule(self, rule: ThreatcodeRule, output_format: Optional[str] = None) -> List[Any]:
        """
        Convert a single Threatcode rule into the target data structure (usually query, see above).
        """
        try:
            ...
            Exception
        ) as e:  # enrich all other exceptions with Threatcode-specific context information
    ...
    def decide_convert_condition_as_in_expression(
        ...
        if not all([isinstance(arg.value, (ThreatcodeString, ThreatcodeNumber)) for arg in cond.args]):
            return False
        ...
        if not self.in_expressions_allow_wildcards and any(
            [
                ...
                if isinstance(arg.value, ThreatcodeString)
            ]
        ):
            return False
    ...
    def convert_condition_field_eq_field(
        self, cond: ThreatcodeFieldReference, state: ConversionState
    ) -> Any:
        ...
        if (
            ...
            if cond.value:
                ...
                return self.convert_condition_not(
                    ConditionNOT(
                        [ConditionFieldEqualsValueExpression(cond.field, ThreatcodeExists(True))],
                        cond.source,
                    ),
                    state,
                )
    ...
    def convert_condition_val(self, cond: ConditionValueExpression, state: ConversionState) -> Any:
        ...
        if isinstance(cond.value, ThreatcodeString):
    ...
    def convert_condition(self, cond: ConditionType, state: ConversionState) -> Any:
        """
        Convert query of Threatcode rule into target data structure (usually query, see above).
    ...
    def finalize_query(
        ...
        conversion of a Threatcode rule results in multiple queries.
    ...
    def finalize_query_default(
        self, rule: ThreatcodeRule, query: Any, index: int, state: ConversionState
    ) -> Any:
        """
...
class TextQueryBackend(Backend):
    ...
    # is one of the flags shortcuts supported by Threatcode (currently i, m and s) and refers to the
    ...
    # Mapping from ThreatcodeRegularExpressionFlag values to static string templates that are used in
    ...
    re_flags: Dict[ThreatcodeRegularExpressionFlag, str] = ThreatcodeRegularExpression.threatcode_to_re_flag
    ...
    # CIDR expressions: define CIDR matching if backend has native support. Else pyThreatcode expands
    ...
    compare_operators: ClassVar[
        Optional[Dict[ThreatcodeCompareExpression.CompareOperators, str]]
    ...
    field_equals_field_expression: ClassVar[
        Optional[str]
    ] = None  # Field comparison expression with the placeholders {field1} and {field2} corresponding to left field and right value side of Threatcode detection item
    ...
    def compare_precedence(self, outer: ConditionItem, inner: ConditionItem) -> bool:
        ...
        # Special case: Conditions containing a ThreatcodeExpansion value convert into OR conditions and therefore the precedence has to be handled the same way.
        if isinstance(
            inner, (ConditionFieldEqualsValueExpression, ConditionValueExpression)
        ) and isinstance(inner.value, ThreatcodeExpansion):
        ...
        return self.field_in_list_expression.format(
            ...
            list=self.list_separator.join(
                [
                    ...
                    if isinstance(arg.value, ThreatcodeString)  # string escaping and qouting
    ...
    def decide_string_quoting(self, s: ThreatcodeString) -> bool:
        """
    ...
    def convert_value_str(self, s: ThreatcodeString, state: ConversionState) -> str:
        """Convert a ThreatcodeString into a plain string which can be used in query."""
        converted = s.convert(
            self.escape_char,
            self.wildcard_multi,
    ...
    def convert_value_re(
        self, r: ThreatcodeRegularExpression, state: ConversionState
    ) -> Union[str, DeferredQueryExpression]:
    ...
    def get_flag_template(self, r: ThreatcodeRegularExpression) -> Dict[str, str]:
        ...
        try:
            return {
                ...
                for flag, c in ThreatcodeRegularExpression.threatcode_to_re_flag.items()
            }
    ...
    def convert_condition_field_eq_val_cidr(
        ...
        cidr: ThreatcodeCIDRExpression = cond.value
        if (
            ...
            expanded_cond = ConditionOR(
                [
                    ConditionFieldEqualsValueExpression(cond.field, ThreatcodeString(network))
